...
$(m,d)$ slice, providing evidence that the minimum Wilf number is already
captured by semigroups of depth~$k^* \le 3$ ($k^*$-\emph{saturation}).
A proof of this saturation would substantially reduce enumeration complexity.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{$k^*$-saturation.}
    The Kunz enumeration shows that for $k^* \le 3$ (covering
    $2.37 \times 10^{10}$ classes), the minimum Wilf number is already
    captured: deeper semigroups ($k^* \ge 4$) never improve the bound.
    Proving this $k^*$-saturation phenomenon would reduce the conjecture
    to a finite-depth verification for each defect.
\end{enumerate}
