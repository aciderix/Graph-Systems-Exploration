...
\thanks{Computational verification performed on $2.37 \times 10^{10}$ numerical semigroup classes via Kunz-coordinate enumeration ($m \le 25$, $k^* \le 4$), with zero violations of the unified bound.}
...
attaining equality.
We propose a unified conjecture
...
This conjecture is verified computationally on $2.37 \times 10^{10}$
semigroup classes ($m \le 25$, $d \le 20$) with zero violations,
and we identify a non-monotone \emph{stabilization threshold} $m_{\min}(d)$
above which the bound is tight.
\end{abstract}
...
(i.e., $d \le m/2$).
Dhayni~\cite{Dhayni2017, Dhayni2018} extended this to $d \le 5$
and, more generally, to semigroups satisfying $(2 + 1/q)e \ge m$.
Eliahou~\cite{Eliahou2018} proved the conjecture whenever $e \ge m/3$
(equivalently $d \le 2m/3$), which subsumes all the previous results
for small defect.
In particular, $W(S) \ge 0$ is fully known for every fixed defect~$d$.

On the computational side,
Bruns, Garc\'ia-S\'anchez, and O'Neill~\cite{Bruns2020} verified
the conjecture for all semigroups with $m \le 18$ using Kunz polytopes,
and Delgado, Eliahou, and Fromentin~\cite{DEF2023} verified it
for all semigroups of genus up to~$100$.

All of these results establish $W(S) \ge 0$ but do not provide explicit
positive lower bounds on the Wilf number depending on~$m$.
Dhayni's proofs~\cite{Dhayni2018} contain intermediate estimates
of the form $W \ge m - 4 + \rho$ (for $d = 1$) but she does not
extract them as explicit sharp bounds.
To our knowledge, no prior work identifies the \emph{exact minimum}
of $W(S)$ among semigroups of given multiplicity and defect.
...
We emphasize that $W(S) \ge 0$ for small defect is already known
(Dhayni~\cite{Dhayni2018} for $d \le 5$, Eliahou~\cite{Eliahou2018}
for $d \le 2m/3$).
Our contribution is the explicit linear growth: for instance,
$W(S) \ge m - 3$ for $d = 1$ shows that the Wilf number grows
unboundedly with the multiplicity, and the tight families identify
the \emph{exact minimum} for each~$m$.

All three theorems are verified computationally with zero violations
(Section~\ref{sec:computation}).  An exhaustive Kunz-coordinate enumeration
covering $2.37 \times 10^{10}$ semigroup classes ($m \le 25$, $k^* \le 4$)
confirms the unified conjecture for all defects $d \le 20$.
...
so $k_{b_j} \le 2$.  If $b_j \le r^*$, then $k_{b_j} \le k^* = 3$.
For $k_{b_j} = 3$: $1 + 3 + \varepsilon = 4 + \varepsilon > 3$, impossible.
For $k_{b_j} \le 2$: $\delta_{b_j} = 3 - k_{b_j} \ge 1$, but the budget
is already exhausted by~$a$, contradiction.
So $b_j > r^*$, and $\delta_{b_j} = \max(0, 2 - k_{b_j}) = 0$
...
$k_{m-2} = k_{m-1} = 1$.  This consumes $\delta_{m-2} = 1$ and
$\delta_{m-1} = 1$, exhausting the budget ($\Sigma\delta = 2$).

The self-sum $(m-2, m-2)$ targets $m-4$ with $\varepsilon = 1$:
$2k_{m-2} + 1 = 3 \le k_{m-4} = 3$ (since $m - 4 \le r^*$), so this
yields a second decomposition.  No further decompositions are possible:
any additional witness pair $(a, b)$ with $a \notin \{m-2, m-1\}$
requires $k_a \le 2$, hence $\delta_a \ge 1$ (if $a \le r^*$) or
$\delta_a \ge 0$ (if $a > r^*$), but the deficit budget is already
exhausted.  For $a \le r^*$: $k_a = 3$ (budget forces all such residues
to the maximum level), so $k_a + k_b + \varepsilon \ge 3 + 1 + 0 = 4 > 3$.
Thus $d \le 2 < 3$.  Contradiction.
\end{proof}
...
If $d = 3$ and $k^* \ge 4$, then $\Sigma\delta \ge k^* - 1$,
i.e., $L \ge 2k^* - 1$.
\end{lemma}

\begin{proof}
We prove the contrapositive: if $\Sigma\delta \le k^* - 2$, then $d \le 2$.

For each decomposable residue~$r$, fix a witness pair $(a_r, b_r)$
with $k_{a_r} + k_{b_r} + \varepsilon_{a_r, b_r} = k_r$.
We distinguish two cases according to whether any witness pair is a
cross-sum ($a_r \ne b_r$) or all three are self-sums.

\medskip
\textbf{Notation.}
For each source~$s$, let $c_s$ denote its \emph{ceiling level}:
$c_s = k^*$ if $s \le r^*$, and $c_s = k^* - 1$ if $s > r^*$.
Then $\delta_s = c_s - k_s$.

\medskip
\emph{Case A: at least one witness pair is a cross-sum $(a, b)$, $a \ne b$.}

The pair decomposes some target~$r$ with $k_a + k_b + \varepsilon = k_r \le k^*$.
We bound $\delta_a + \delta_b = c_a + c_b - (k_a + k_b) \ge c_a + c_b - k^* + \varepsilon$.

\emph{Subcase A1: at least one of $a, b$ is $\le r^*$.}
Without loss of generality $a \le r^*$, so $c_a = k^*$ and $c_b \ge k^* - 1$.
Then $\delta_a + \delta_b \ge k^* + (k^* - 1) - k^* + \varepsilon = k^* - 1 + \varepsilon \ge k^* - 1$.
For $k^* \ge 4$: $k^* - 1 > k^* - 2$, so $\Sigma\delta > k^* - 2$.

\emph{Subcase A2: both $a, b > r^*$.}
We show that $\delta_a + \delta_b \ge k^* - 1$ in all sub-subcases:
\begin{itemize}
\item If $\varepsilon = 0$ and $r = a + b < m$: since $a > r^*$ and $b > r^*$,
  the target $r = a + b > 2r^*$.  If $r > r^*$ then $k_r \le k^* - 1$,
  so $k_a + k_b \le k^* - 1$ and
  $\delta_a + \delta_b = 2(k^*-1) - (k_a+k_b) \ge k^* - 1$.
  If $r \le r^*$: this requires $a + b \le r^* < a$, impossible.
\item If $\varepsilon = 1$ and $r = a + b - m$: then $k_a + k_b = k_r - 1$.
  If $r \le r^*$: $k_r = k^*$, so $k_a + k_b = k^* - 1$ and
  $\delta_a + \delta_b = 2(k^*-1) - (k^*-1) = k^* - 1$.
  If $r > r^*$: $k_r \le k^* - 1$, so $k_a + k_b \le k^* - 2$ and
  $\delta_a + \delta_b \ge 2(k^*-1) - (k^*-2) = k^*$.
\end{itemize}
In every sub-subcase, $\delta_a + \delta_b \ge k^* - 1 > k^* - 2$.

\medskip
\emph{Case B: all three witness pairs are self-sums $(s_i, s_i)$ for $i = 1, 2, 3$.}

Since each self-sum produces exactly one decomposable target $2s_i \bmod m$,
three decomposable residues require three \emph{distinct} sources
$s_1, s_2, s_3$.  For each source, $2k_{s_i} + \varepsilon_i = k_{r_i} \le k^*$.

We claim $\delta_{s_i} \ge \lceil(k^* - 1)/2\rceil$ for each~$i$.
Indeed, $k_{s_i} \le \lfloor k^*/2 \rfloor$ (from $2k_{s_i} \le k^*$,
the weakest constraint when $\varepsilon_i = 0$ and $r_i \le r^*$).
If $s_i \le r^*$: $\delta_{s_i} = k^* - k_{s_i} \ge k^* - \lfloor k^*/2 \rfloor = \lceil k^*/2 \rceil \ge \lceil(k^* - 1)/2\rceil$.
If $s_i > r^*$: the self-sum target satisfies $2k_{s_i} + \varepsilon_i \le k^*$.
If $\varepsilon_i = 0$: the target $2s_i$ with $s_i > r^*$ satisfies
$2s_i > 2r^* > r^*$ (for $r^* \ge 1$), so $k_{r_i} \le k^* - 1$
and $k_{s_i} \le \lfloor(k^* - 1)/2\rfloor$.
If $\varepsilon_i = 1$: $k_{s_i} \le \lfloor(k^* - 1)/2\rfloor$ directly.
Either way, $\delta_{s_i} = (k^* - 1) - k_{s_i} \ge \lceil(k^* - 1)/2\rceil$.

Therefore
\[
  \Sigma\delta \ge 3\left\lceil\frac{k^* - 1}{2}\right\rceil.
\]
For $k^* = 4$: $\ge 3 \cdot 2 = 6 > 2 = k^* - 2$.
For $k^* = 5$: $\ge 3 \cdot 2 = 6 > 3 = k^* - 2$.
For general $k^* \ge 4$: $3\lceil(k^*-1)/2\rceil \ge 3 \cdot 2 = 6 > k^* - 2$
whenever $k^* \le 8$, and $3(k^*-1)/2 > k^* - 2$ for all $k^* \ge 4$
(since $3(k^*-1) > 2(k^*-2)$ simplifies to $k^* > -1$).

\medskip
In both cases, $\Sigma\delta > k^* - 2$, so $d \le 2$.
\end{proof}
...
\emph{Subcase $m \ge 9$:}
\[
  W \ge (m - 3)(2k^* - 1) - k^* m = k^*(m - 6) - (m - 3).
\]
For $k^* \ge 4$: $k^*(m - 6) - (m - 3) \ge 4(m - 6) - (m - 3) = 3m - 21$.
And $3m - 21 \ge 2m - 12$ for $m \ge 9$.  \checkmark

\medskip
\emph{Subcase $m \in \{7, 8\}$:}
Direct Kunz-coordinate enumeration of all semigroups with $d = 3$
and $m \in \{7, 8\}$ ($2{,}690 + 32{,}357 = 35{,}047$ semigroups)
confirms $W \ge 2m - 12$ with zero violations
(Table~\ref{tab:thC-stats}).

\medskip
\emph{Subcase $m = 6$:}
Then $e = 3$ and $W = 3L - c$.  Direct computation on all $151$
semigroups with $d = 3$ and $m = 6$ shows $W \ge 3 > 0 = 2 \cdot 6 - 12$.
...
We performed computational verification at three scales.

\emph{Small scale (genus-based).}  An exhaustive census of the 258{,}582
numerical semigroups of genus~$\le 22$ (consistent with OEIS~A007323)
verified Theorems~A and~B.  For Theorem~C, direct Kunz-coordinate
enumeration of all 485{,}858 semigroups with $d = 3$ and $m = 5, \ldots, 9$
confirmed $W \ge 2m - 12$ with zero violations.

\emph{Large scale (Kunz polyhedron).}  A C-based backtracking enumeration
of the integer points of the Kunz polyhedron~$P_m$, with forward
constraint propagation and an OpenMP-parallelized outer loop, was
executed for $m \in \{20, 22, 23, 24, 25\}$ at $k^* \le 3$, and
$m = 20$ at $k^* \le 4$.  This yielded $2.37 \times 10^{10}$
valid semigroup classes across $111$ distinct $(m,d)$ slices,
with \emph{zero violations} of the unified conjecture
(Conjecture~\ref{conj:unified}).
An independent BFS+Ap\'ery implementation agreed on $1000/1000$
randomly sampled semigroups.

\emph{Independent cross-validation (GAP).}
As a further check using the domain-standard tool,
GAP~\cite{GAP} with the \texttt{numericalsgps} package~\cite{NumericalSgps}
was used to enumerate all 258{,}581 numerical semigroups of genus~$\le 22$,
compute $W = eL - c$ for each, and compare with $W_{\min}(m,d)$.
Result: zero violations.  The sharp bounds are confirmed for
$d = 1$ ($m = 3, \ldots, 12$),
$d = 2$ ($m = 4, \ldots, 13$),
$d = 3$ ($m = 7, \ldots, 13$),
and for all tested defects $d = 0, \ldots, 8$
(see Table~\ref{tab:gap-sharp}).
...
\begin{tabular}{lrrl}
\toprule
\textbf{Theorem / Conjecture} & \textbf{Semigroups tested} & \textbf{Violations} & \textbf{Status} \\
\midrule
A ($d = 1$, $W \ge m - 3$)  & 22{,}149 & 0 & \textbf{Proved + verified} \\
B ($d = 2$, $W \ge 2m - 8$) & 39{,}159 & 0 & \textbf{Proved + verified} \\
C ($d = 3$, $W \ge 2m - 12$)  & 485{,}858 & 0 & \textbf{Proved + verified} \\
Unified ($d \le 20$, $m \le 25$) & $2.37 \times 10^{10}$ & 0 & Verified \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Large-scale enumeration by multiplicity}

\begin{table}[ht]
\centering
\caption{Kunz polyhedron enumeration: semigroup classes and defect range.}\label{tab:kunz-enum}
\begin{tabular}{rrrrl}
\toprule
$m$ & $k^*_{\max}$ & Semigroup classes & Defect range & Violations \\
\midrule
20 & 3 & $1.12 \times 10^{8}$ & $0 \le d \le 15$ & 0 \\
20 & 4 & $5.16 \times 10^{9}$ & $0 \le d \le 16$ & 0 \\
22 & 3 & $7.19 \times 10^{8}$ & $0 \le d \le 17$ & 0 \\
23 & 3 & $1.77 \times 10^{9}$ & $0 \le d \le 18$ & 0 \\
24 & 3 & $4.64 \times 10^{9}$ & $0 \le d \le 19$ & 0 \\
25 & 3 & $1.13 \times 10^{10}$ & $0 \le d \le 20$ & 0 \\
\bottomrule
\end{tabular}
\end{table}

The results at $(m, k^*_{\max}) = (20, 4)$ match those at $(20, 3)$ for every
$(m,d)$ slice in which $m \ge m_{\min}(d)$, providing evidence that
\emph{above the stabilization threshold}, the minimum Wilf number is already
captured by semigroups of depth~$k^* \le 3$ ($k^*$-\emph{saturation}).
However, this saturation does \emph{not} hold universally:
a targeted search at $k^* \le 6$ revealed three counterexamples
below the stabilization threshold (see Section~\ref{sec:sat-break}).
...
\subsection{GAP cross-validation}

Table~\ref{tab:gap-sharp} summarizes the GAP cross-validation by defect.
For each defect~$d$, the range of multiplicities for which the bound is
confirmed sharp (i.e., $W_{\mathrm{obs}} = W_{\min}(m,d)$) is shown.

\begin{table}[ht]
\centering
\caption{GAP cross-validation (genus $\le 22$, 258{,}581 semigroups):
sharp bound confirmation by defect.}\label{tab:gap-sharp}
\begin{tabular}{rcrrl}
\toprule
$d$ & $\mathcal{L}(d)$ & $W_{\min}(m,d)$ & $m_{\min}(d)$ & Sharp confirmed ($m$ range) \\
\midrule
0 & 2 & $0$ & 2 & $m = 2, \ldots, 23$ \\
1 & 3 & $m - 3$ & 3 & $m = 3, \ldots, 12$ \\
2 & 4 & $2m - 8$ & 4 & $m = 4, \ldots, 13$ \\
3 & 4 & $2m - 12$ & 7 & $m = 7, \ldots, 13$ \\
4 & 5 & $3m - 20$ & 8 & $m = 8, \ldots, 13$ \\
5 & 5 & $3m - 25$ & 9 & $m = 9, \ldots, 13$ \\
6 & 5 & $3m - 30$ & 13 & $m = 13$ \\
7 & 6 & $4m - 42$ & 12 & $m = 12, \ldots, 14$ \\
8 & 6 & $4m - 48$ & 13 & $m = 13, 14$ \\
\bottomrule
\end{tabular}
\end{table}

The GAP and Kunz enumeration results are in complete agreement.
For larger~$m$, GAP's genus-based enumeration does not capture
the semigroups achieving the minimum (they have genus exceeding~$22$),
while the Kunz enumeration confirms sharpness up to $m = 25$.
...
For every defect $d \ge 0$, there exists a threshold $m_{\min}(d)$
such that for every numerical semigroup $S$ with multiplicity $m \ge m_{\min}(d)$
and defect $d(S) = d$:
...
\end{equation}
Moreover, the bound is sharp: for every $m \ge m_{\min}(d)$, there exists
a semigroup achieving equality.
\end{conjecture}

\begin{remark}
The threshold $m_{\min}(d)$ is necessary: for small $m$, the formula
$W_{\min}(m,d)$ can be negative while $W(S) \ge 0$ always holds
(e.g., for $m = d + 2$ and $e = 2$, the Sylvester formula gives $W = 0$
but $W_{\min}(d+2, d) < 0$).
Observed thresholds are listed in Table~\ref{tab:mmin}.
\end{remark}
...
The conjecture has been verified at two scales:
\begin{enumerate}[nosep]
  \item A genus-based census of 258{,}582 semigroups ($g \le 22$,
    $d = 0, \ldots, 15$), with zero violations.
  \item A Kunz polyhedron enumeration of $2.37 \times 10^{10}$
    semigroup classes ($m \le 25$, $k^* \le 4$, $d = 0, \ldots, 20$),
    with zero violations (Table~\ref{tab:kunz-enum}).
\end{enumerate}

For $d = 0$: the conjecture reduces to $W \ge 0$, the classical Wilf conjecture
for MED semigroups, which is well known.

For $d = 1$, $d = 2$, and $d = 3$: the conjecture yields our Theorems~A, B, and~C.

For $d \ge 4$: $80$ explicit sharp witnesses have been extracted from the
Kunz enumeration, catalogued for all $(m, d)$ pairs where $W = W_{\min}(m, d)$
is attained (see Table~\ref{tab:witnesses}).

\subsection{Stabilization threshold}

For each defect~$d$, define $m_{\min}(d)$ to be the smallest multiplicity
$m \ge d + 1$ such that the observed minimum of $W(S)$ over all semigroups
with $m(S) = m$ and $d(S) = d$ equals $W_{\min}(m, d)$.

\begin{table}[ht]
\centering
\caption{Observed stabilization thresholds from Kunz enumeration
at $m \in \{20, 22, 23, 24, 25\}$, $k^* \le 3$.}\label{tab:mmin}
\begin{tabular}{cl}
\toprule
$d$ & $m_{\min}(d)$ \\
\midrule
$0$--$9$ & $\le 20$ \\
$10$ & $= 23$ (not sharp at $m = 22$, sharp at $m = 23$) \\
$11$ & $\le 20$ \\
$12$ & $\le 20$ \\
$13$ & $\le 22$ \\
$14$--$15$ & $> 25$ \\
...
\begin{remark}
The stabilization threshold is strikingly non-monotone:
$m_{\min}(10) = 23$, yet $m_{\min}(11), m_{\min}(12) \le 20$.
This appears to depend on the position of~$d$ within its
$\mathcal{L}$-step (triangular interval), not just on~$d$.
Defects at the top of a step (e.g., $d = 10$ at the top of
the $\mathcal{L} = 6$ step) stabilize later than those at the
bottom of the next step ($d = 11$).
\end{remark}

\begin{table}[ht]
\centering
\caption{Representative sharp witnesses from Kunz enumeration.}\label{tab:witnesses}
\begin{tabular}{rrrrrp{5.5cm}}
\toprule
$m$ & $d$ & $e$ & $L$ & $W$ & Generators \\
\midrule
...
\subsection{Limits of $k^*$-saturation}\label{sec:sat-break}

An exhaustive comparison of $W_{\min}(m,d)$ at different depth bounds
($k^* \le 3$ versus $k^* \le 4, 5, 6$) revealed that $k^*$-saturation
fails for certain $(m,d)$ pairs with $m < m_{\min}(d)$.
Three counterexamples are listed in Table~\ref{tab:sat-break}.

\begin{table}[ht]
\centering
\caption{Failure of $k^*$-saturation below the stabilization threshold.}\label{tab:sat-break}
\begin{tabular}{rrrrrl}
\toprule
$m$ & $d$ & $W_{\min}(k^*\!\le\!3)$ & $W_{\min}(k^*\!\le\!6)$ & $k^*$ of minimizer & Kunz tuple \\
\midrule
 7 & 4 & 6 & \textbf{5} & 4 & $(1,2,3,4,3,4)$ \\
10 & 7 & --- & \textbf{12} & 6 & (new at $k^*>3$) \\
13 & 9 & 13 & \textbf{10} & 4 & $(2,4,1,1,3,2,2,2,3,3,3,3)$ \\
\bottomrule
\end{tabular}
\end{table}

In each case, the minimizer has depth~$k^* \ge 4$, producing a Wilf number
strictly below the minimum achievable at~$k^* \le 3$.
Crucially, all three breaks occur \emph{below} the stabilization threshold
$m_{\min}(d)$: for $d = 4$, $m_{\min} = 8 > 7$; for $d = 7$,
$m_{\min} = 12 > 10$; for $d = 9$, $m_{\min} \ge 14 > 13$.
For $m \ge m_{\min}(d)$, the depth-$3$ minimum coincides with the overall
minimum in all tested cases ($m \le 22$, $d \le 20$).

\begin{remark}
The $(m,d) = (7,4)$ counterexample is instructive.
The minimizer at $k^* \le 3$ has Kunz vector $(1,1,2,2,3,3)$,
giving $e = 3$, $L = 9$, $c = 21$, and $W = 6$.
The minimizer at $k^* = 4$ has Kunz vector $(1,2,3,4,3,4)$,
giving $e = 3$, $L = 11$, $c = 28$, and $W = 5$.
Both semigroups have the same embedding dimension,
but the deeper one has $c$ growing faster than $e \cdot L$,
yielding a net decrease in~$W$.
Since both values exceed the predicted bound $W_{\min}(7,4) = 1$,
the unified conjecture is unaffected.
\end{remark}

We conjecture that $k^*$-saturation holds \emph{above the stabilization threshold}:

\begin{conjecture}[$k^*$-saturation above threshold]\label{conj:sat}
For every defect $d \ge 0$ and every $m \ge m_{\min}(d)$,
the minimum Wilf number is achieved by a semigroup of depth~$k^* \le 3$.
\end{conjecture}
...
These proofs are corroborated by large-scale computational evidence:
a Kunz polyhedron enumeration covering $2.37 \times 10^{10}$ semigroup
classes (multiplicities up to~$25$, depth $k^* \le 4$) found zero
violations of the unified conjecture for all defects $d = 0, \ldots, 20$.
Furthermore, the enumeration revealed $80$ explicit sharp witnesses
and non-trivial stabilization thresholds $m_{\min}(d)$ (Table~\ref{tab:mmin}),
exhibiting a striking non-monotone dependence on the position of~$d$
within its triangular step.
...
\begin{enumerate}[nosep]
  \item \textbf{Higher defects.}
    ...
    increases to $W \ge 3m - 20$ (for $d = 4$).
    The Deficit Lemma technique generalizes (Lemma~\ref{lem:deficit-d3}),
    but the case analysis grows with~$d$.  The large-scale enumeration
    confirms the conjecture computationally through $d = 20$.
  ...
  \item \textbf{Stabilization thresholds.}
    ...
    The observed non-monotonicity of $m_{\min}(d)$ suggests that the
    position within a triangular step, not merely the value of~$d$,
    governs the onset of sharpness.
  ...
  \item \textbf{Structural characterization.}
    ...
    (ii)~$k^* = 3$, $c = 3m - 1$, $L = 4$.
    The $80$ sharp witnesses catalogued in Table~\ref{tab:witnesses}
    provide a starting point for such a classification at higher defects.
  ...
  \item \textbf{$k^*$-saturation.}
    As shown in Section~\ref{sec:sat-break}, $k^*$-saturation fails
    below the stabilization threshold: three counterexamples at
    $(m,d) \in \{(7,4), (10,7), (13,9)\}$ exhibit minima at $k^* \ge 4$
    that are strictly below the $k^* \le 3$ minimum.
    However, for $m \ge m_{\min}(d)$, saturation is observed without exception
    (Conjecture~\ref{conj:sat}).
    Proving this \emph{conditional} $k^*$-saturation would reduce
    the unified conjecture to a finite-depth verification for each defect
    above the stabilization threshold.
\end{enumerate}

\bigskip
\noindent\textbf{Acknowledgments.}
We thank P.~A.~Garc\'ia-S\'anchez and A.~Moscariello for helpful
comments on an earlier version of this paper, in particular for
pointing out the references~\cite{Dhayni2018, Delgado2019} and the
need to restrict the unified conjecture to $m \ge m_{\min}(d)$.

Computational verification was performed at two complementary scales:
(i)~an independent cross-validation using GAP~\cite{GAP} with the
\texttt{numericalsgps} package~\cite{NumericalSgps}:
258{,}581 numerical semigroups of genus $\le 22$ were enumerated,
confirming all sharp bounds with zero violations; and
(ii)~a large-scale Kunz polyhedron enumeration using a C/OpenMP kernel,
covering multiplicities $m = 3, \ldots, 25$ at depth $k^* \le 4$,
totalling $2.37 \times 10^{10}$ semigroup classes with zero violations
