...
\begin{definition}[Kunz coordinates {\cite{Kunz1987}}]\label{def:kunz}
...
the Frobenius number is $F = ma - m - a$ (Sylvester~\cite{Sylvester1884}),
the genus is $g = (m-1)(a-1)/2$, and therefore $L = c - g = c/2$.
Since $e = 2$, the Wilf number is
\begin{equation}\label{eq:two-gen}
  W(S) = 2L - c = 2 \cdot \frac{c}{2} - c = 0.
\end{equation}
That is, every two-generator numerical semigroup has $W(S) = 0$.
...
By~\eqref{eq:two-gen}, $W = 0 = 2 \cdot 4 - 8$.
\end{proof}
...
enumerated via the tree method of Bras-Amor\'os~\cite{BrasAmoros2008}.
...
(i)~$k^* = 2$, $L = 3$, $c = 2m$ (the $T_m$ family, for $m \ge 4$);
and (ii)~$k^* = 3$, $L = 4$, $r^* \le m - 2$ (for small~$m$).
An explicit tight example is
...
Figure~\ref{fig:scatter} displays the Wilf number of all $d = 1$ and $d = 2$
semigroups from the census, together with the sharp lower bounds.
Figure~\ref{fig:fan} shows the fan of minimal Wilf numbers by defect,
confirming the unified conjecture.
Figure~\ref{fig:slack} shows the distribution of slack $W - (m - 3)$
for $d = 1$, illustrating that most semigroups lie far above the bound.

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig1_scatter_bounds.png}
\caption{Sharp lower bounds on the Wilf number by defect.
  Each dot is a numerical semigroup from the genus $\le 22$ census.
  The solid line is Theorem~A ($W = m - 3$), the dashed line is Theorem~B ($W = 2m - 8$).}
\label{fig:scatter}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig2_wmin_fan.png}
\caption{Fan of minimal Wilf numbers by defect.
  Solid lines connect observed $W_{\min}(m, d)$; dotted lines show the
  predicted formula $(m - d) \cdot \mathcal{L}(d) - 2m$.}
\label{fig:fan}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{fig3_slack_distribution.png}
\caption{Distribution of slack $W(S) - (m - 3)$ for $d = 1$ semigroups.
  The dashed red line marks the tight bound (slack $= 0$).}
\label{fig:slack}
\end{figure}
