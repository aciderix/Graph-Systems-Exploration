...
W(S) := e·L − c ≥ 0. Writing d = m − e for the \emph{defect}, we
propose an explicit lower bound

\begin{center}
W(S) ≥ W\_min(m,d) := (m − d)·L(d) − 2m, \qquad
L(d) = ⌈(√(8d+1) − 1) / 2⌉ + 2,
\end{center}
...
The Kunz coordinates $(k_1,...,k_{m-1})$ of $S$ are defined by
...
For every numerical semigroup S with m(S) = m ≥ 2 and d(S) = d in the
range 0, 1, ..., m−1,

\begin{center}
W(S) ≥ W\_min(m,d) := (m − d)·L(d) − 2m,
\end{center}

where L(d) is defined by: L(0) = 2, and for d ≥ 1,
L(d) = ⌈(√(8d+1) − 1) / 2⌉ + 2.
\end{conjecture}

\begin{remark}
L(d) is the unique increasing step function such that L(d) = ℓ + 2
on the triangular interval d in [ℓ(ℓ−1)/2 + 1, ℓ(ℓ+1)/2].
Its appearance is not arbitrary: in each step, the bound corresponds
to Apery sets concentrated at k* in {ℓ+1, ℓ+2}, and the
triangular numbers mark the exact defects at which k* must
increase.
\end{remark}
...
For each defect d we define m\_min(d) to be the smallest m ≥ d+1
such that the minimum of W(S) over all numerical semigroups with
m(S)=m and d(S)=d equals W\_min(m,d).
...
\begin{tabular}{c|l}
\toprule
$d$ & best known bound on $m_{\min}(d)$ from $m \in \{20,22,23,24,25\}$ enumeration\\
\midrule
0--9  & $\le 20$ \\
10    & $= 23$ (not sharp at $m=22$, sharp at $m=23$)\\
11    & $\le 20$ \\
12    & $\le 20$ \\
13    & $\le 22$ (sharp at $m=22$)\\
14    & $> 25$ (not sharp up to $m=25$)\\
15    & $> 25$ \\
16    & $= 25$ (not sharp at $m=24$, sharp at $m=25$)\\
17--20 & $> 25$ \\
\bottomrule
\end{tabular}
\caption{Observed stabilization thresholds from complete enumerations
at $m \in \{20,22,23,24,25\}$ with $K_{\max}=3$ (total: $2.3 \times
10^{10}$ semigroup classes). Note the striking non-monotonicity:
$m_{\min}(10)=23$, then $m_{\min}(11),m_{\min}(12) \le 20$, then
$m_{\min}(13) \le 22$. Defects $d=11,12$ stabilize earlier than
$d=10$; defect $d=16$ stabilizes earlier than $d=14,15$. The
complete $m_{\min}$ function appears to depend on the position of
$d$ within its $L$-step, not just on $d$.}
\label{tab:mmin}
\end{table}

\begin{remark}
The earlier draft of this manuscript conjectured a smooth growth
$m_{\min}(d) = 2d + O(\sqrt d)$. The m=25 data falsifies this: for
$d=11$ we have $m_{\min}(11) \le 20 < 2d = 22$, and within each
$L$-step (e.g.\ $d \in \{11,\ldots,15\}$ for $L=7$) $m_{\min}$ is
non-monotone, dipping before the top of the step and then rising
sharply. A refined conjecture on the shape of $m_{\min}$ is left as
open.
\end{remark}
...
\midrule
20 & 2 & 18 & 4 & 32 & {\textless}20,21,22,45,...{\textgreater}\\
20 & 9 & 11 & 6 & 26 & {\textless}20,21,22,26,29,45,53,...{\textgreater}\\
22 & 5 & 17 & 5 & 41 & {\textless}22,23,24,27,52,...{\textgreater}\\
23 &10 & 13 & 6 & 32 & {\textless}23,24,27,32,34,49,...{\textgreater}\\
24 & 5 & 19 & 5 & 47 & {\textless}24,25,26,29,56,...{\textgreater}\\
24 &13 & 11 & 7 & 29 & {\textless}24,25,27,28,33,35,64,...{\textgreater}\\
\bottomrule
\end{tabular}
\caption{Representative sharp witnesses. Each row satisfies
W = W\_min(m,d) exactly.}
\label{tab:witnesses}
\end{table}
...
$k_1, k_2, ..., k_{m-1}$, forward-checking the ``no-carry''
