"""Extended verification d=2..5, pushing m as high as feasible."""
import time, sys
...
print("=" * 70)
print("EXTENDED VERIFICATION: d=2..5, m=15..25")
print("=" * 70)
...
for d in range(4, 6):
    ...
    for m in range(15, 26):
        target_e = m - d
        if target_e < 2: continue
        predicted = (m - d) * L_d - 2 * m
        
        # Generic minimizer has g = 2m - L_d, so need genus >= that + margin
        eff_max_g = min(2*m + 5, 50)
        ...
        if tc == 0:
            ...
            if elapsed > 60:
                print(f"  ⏱️ Too slow, stopping d={d}")
                break
            continue
        
        w_min = min(r[4] for r in results)
        achiever = min(results, key=lambda r: r[4])
        violations = sum(1 for r in results if r[4] < predicted)
        match = "EXACT" if w_min == predicted else f"+{w_min-predicted}"
